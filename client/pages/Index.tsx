...
import { loadSiteData, type SiteData } from "@/lib/links-loader";
...
export default function Index() {
  ...
  useEffect(() => {
    ...
    loadSiteData().then((data) => {
      // Cosmic Videos
      if (data.cosmicVideos?.length) {
        setCosmicVideos(data.cosmicVideos.map(v => v.url || ""));
        setActiveCosmicVideosList(data.cosmicVideos.map(v => v.active !== false));
      }
      
      // Playlist Videos
      if (data.playlistVideos?.length) {
        setPlaylistVideos(data.playlistVideos.map(v => v.url ? convertToEmbedUrl(v.url) : ""));
      }
      
      // Feel Cosmos Videos
      if (data.feelCosmosVideos?.length) {
        setFeelCosmosVideos(data.feelCosmosVideos.map(v => v.url ? convertToEmbedUrl(v.url) : ""));
        setActiveFeelCosmosVideosList(data.feelCosmosVideos.map(v => v.active !== false));
      }
      
      // Feel Cosmos Songs
      if (data.feelCosmosSongs?.length) {
        setFeelCosmosSongs(data.feelCosmosSongs.filter(s => s.title || s.url).map(s => ({ title: s.title, url: s.url })));
      }
      
      // Podcast Videos
      if (data.podcastVideos?.length) {
        setPodcastVideos(data.podcastVideos.map(v => v.url ? convertToEmbedUrl(v.url) : ""));
        setActivePodcastVideosList(data.podcastVideos.map(v => v.active !== false));
      }
      
      // NFT Videos
      if (data.nftVideos?.length) {
        setNftVideos(data.nftVideos.map(v => v.url ? convertToEmbedUrl(v.url) : ""));
        setActiveNftVideosList(data.nftVideos.map(v => v.active !== false));
      }
      
      // NFT Collections
      if (data.nftCollections?.length) {
        setNftCollections(data.nftCollections.map(c => c.url || ""));
        setNftCollectionNames(data.nftCollections.map(c => c.name || ""));
        setNftCollectionCustomImages(data.nftCollections.map(c => c.customImage || ""));
        setActiveNftCollectionsList(data.nftCollections.map(c => c.active !== false));
      }
      
      // Cosmic Ambient Videos
      if (data.cosmicAmbientVideos?.length) {
        setCosmicAmbientVideos(data.cosmicAmbientVideos.map(v => v.url ? convertToEmbedUrl(v.url) : ""));
        setActiveCosmicAmbientVideosList(data.cosmicAmbientVideos.map(v => v.active !== false));
      }
      
      // Settings
      if (data.settings) {
        if (data.settings.aboutMeText) setAboutMeText(data.settings.aboutMeText);
        if (data.settings.learnMoreText) setLearnMoreText(data.settings.learnMoreText);
        if (data.settings.selectedBackground) setSelectedBackground(data.settings.selectedBackground);
        setIsContactEnabled(data.settings.contactEnabled !== false);
        setIsHeroModalsEnabled(data.settings.heroModalsEnabled !== false);
        if (data.settings.playlistVideosActive !== undefined) setActivePlaylistVideos(data.settings.playlistVideosActive);
        if (data.settings.playlistSongsActive !== undefined) setActivePlaylistSongs(data.settings.playlistSongsActive);
        if (data.settings.feelCosmosSongsActive !== undefined) setActiveFeelCosmosSongs(data.settings.feelCosmosSongsActive);
      }
    });
  }, []);
